...
class FineLoader(Dataset):
    def __init__(self, data_path):
        self.tokenizer = AutoTokenizer.from_pretrained(
            "klue/roberta-base", truncation=True, padding="max_length", max_length=512
        )
    ...
    def __getitem__(self, idx):  # 기본적인 구성
        ...
        for utt in context:
            context_token += self.tokenizer.encode(
        ...
        pos_respons_token += self.tokenizer.encode(
            positive_response, add_special_tokens=False, max_length=512, truncation=True
        )
        ...
        for negative_response in negative_responses:
            ...
            neg_respons_token += self.tokenizer.encode(
                negative_response,
    ...
    def collate_fn(self, sessions):
        ...
        return:
            ...
            batch_labels: (B)
        """
        max_input_len = 50  # 최대 길이를 512로 설정
        ...
        for session in sessions:
            ...
            for session_token in session_tokens:
                # 최대 길이를 초과하는 토큰은 잘라냄
                if len(session_token) > max_input_len:
                    session_token = session_token[:max_input_len]
        ...
        return (
            torch.tensor(batch_input_tokens),
            torch.tensor(batch_input_attentions),
            torch.tensor(batch_input_labels),
        )
